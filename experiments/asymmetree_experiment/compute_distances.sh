...
for T in {10,20,30,40,50,60,70,80,90}; do 
    ...
    for ((i=0; i<NRUNS;i++)); do 
        ...
        for ((j=0; j < NRECONCRUNS; j++)); do 
            ...
            ~/Documents/meta_rep/postdoc_projects/transfer_edition_distance/ted_module/target/release/ted_module --weighted --unordered networks/network_${NLEAVES}leaves_${i}_unordered.gr ranger_dtl_output/ranger_dtl_reconciliation_${NLEAVES}leaves_${i}_${j}_T${T}.gr >> "results_nleaves${NLEAVES}_nruns${NRUNS}_nreconcruns${NRECONCRUNS}_T${T}"; done; done; done
#for ((i=0; i<NRUNS;i++)); do 
