...
#            python3 total_weight.py networks/network_${NLEAVES}leaves_${i}_unordered.gr ranger_dtl_output/ranger_dtl_reconciliation_${NLEAVES}leaves_${i}_${j}_T${T}.gr >> "results_nleaves${NLEAVES}_nruns${NRUNS}_nreconcruns${NRECONCRUNS}_T${T}";
            ~/meta_rep/postdoc_projects/transfer_edition_distance/ted_module/target/release/ted_module --unordered networks/network_${NLEAVES}leaves_${i}_unordered.gr ranger_dtl_output/ranger_dtl_reconciliation_${NLEAVES}leaves_${i}_${j}_T${T}.gr >> "results_nleaves${NLEAVES}_nruns${NRUNS}_nreconcruns${NRECONCRUNS}_T${T}"; done; done; done
#for ((i=0; i<NRUNS;i++)); do 
#    echo "aggregated network ${NLEAVES} num ${i}" >> "results_nleaves${NLEAVES}_nruns${NRUNS}_nreconcruns${NRECONCRUNS}"; 
#    python3 total_weight.py networks/network_${NLEAVES}leaves_${i}_unordered.gr weighted_reconstructions/weighted_network${NLEAVES}leaves_${i}.wgr >> "results_nleaves${NLEAVES}_nruns${NRUNS}_nreconcruns${NRECONCRUNS}"; 
